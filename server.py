...
GOVS_ASSETS_DIR = os.path.join(ROOT_DIR, 'assets')
GOVS_ASSETS_PATH = os.path.join(GOVS_ASSETS_DIR, 'governorates.json')
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/api/save-menu':
            ...
            try:
                # يدعم شكلين: مصفوفة عامة قديمة، أو كائن بفرع محدد
                ...
                for g in (items if isinstance(items, list) else []):
                    name = (g.get('name') if isinstance(g, dict) else '').strip()
                    fee = g.get('fee') if isinstance(g, dict) else None
                    try:
                        fee = int(fee) if fee is not None else 0
                    except Exception:
                        fee = 0
                    if name:
                        clean.append({'name': name, 'fee': fee})

                os.makedirs(GOVS_ASSETS_DIR, exist_ok=True)

                # حفظ حسب الفرع إن وجد، وإلا إلى الملف العام القديم
                if branch:
                    safe_branch = re.sub(r'[^a-z0-9_-]+', '-', branch)
                    target = os.path.join(GOVS_ASSETS_DIR, f'governorates.{safe_branch}.json')
                else:
                    target = GOVS_ASSETS_PATH

                with open(target, 'w', encoding='utf-8') as f:
                    json.dump(clean, f, ensure_ascii=False, indent=2)

                # حفظ حالة التفعيل مجمعة في ملف مستقل
                if branch is not None and enabled is not None:
                    en_path = os.path.join(GOVS_ASSETS_DIR, 'governorates.enabled.json')
                    try:
                        existing = {}
                        if os.path.exists(en_path):
                            with open(en_path, 'r', encoding='utf-8') as ef:
                                existing = json.load(ef) or {}
                        existing[branch] = bool(enabled)
                ...
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.end_headers()
                rel = target.replace(ROOT_DIR, '').lstrip('\\/').replace('\\', '/')
...
def main():
    port = int(os.environ.get('PORT', '1780'))
